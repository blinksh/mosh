...
void iOSClient::shutdown( void )
{
  ...
  if ( still_connecting() ) {
    ...
    fprintf(
      out_fd, "Please verify that UDP port %s is not firewalled and can reach the server.\r\n\n", port.c_str() );
    fprintf( out_fd,
             "(By default, mosh uses a UDP port between 60000 and 61000. The -p option\nselects a specific UDP "
             "port number.)\r\n" );
  } else if ( network ) {
    if ( !clean_shutdown ) {
      fprintf( out_fd,
               "\r\n\nmosh did not shut down cleanly. Please note that the\r\nmosh-server process may still be "
...
void iOSClient::main_init( const std::string encoded_state )
{
  ...
  if ( encoded_state.size() > 0 && context.ParseFromString( encoded_state ) ) {
    ...
    network = new Network::Transport<Network::UserStream, Terminal::Complete>(
      blank, local_terminal, key.c_str(), ip.c_str(), port.c_str(), sent_states, received_states );
  } else {
    network = new Network::Transport<Network::UserStream, Terminal::Complete>(
      blank, local_terminal, key.c_str(), ip.c_str(), port.c_str() );
  }
    ...
    try {
      ...
      if ( sel.signal( SIGTERM ) || sel.signal( SIGINT ) || sel.signal( SIGHUP ) || sel.signal( SIGPIPE ) ) {
        ...
        if ( !network->has_remote_addr() ) {
          ...
          overlays.get_notification_engine().set_notification_string(
            wstring( L"Signal received, shutting down..." ), true );
          network->start_shutdown();
        }
      }
      ...
      if ( still_connecting() && ( !network->shutdown_in_progress() )
        ...
        if ( timestamp() - network->get_latest_remote_state().timestamp > 15000 ) {
          if ( !network->shutdown_in_progress() ) {
            overlays.get_notification_engine().set_notification_string(
              wstring( L"Timed out waiting for server..." ), true );
            network->start_shutdown();
          }
        } else {
